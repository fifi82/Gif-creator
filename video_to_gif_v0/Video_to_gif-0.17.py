...
def f_ouvrir_video(): # Bouton ouvrir
    global frames, image_en_cours, nb_frame, a_debut, b_fin, image_lecture,tpi, tpi_s, _charge,nom,ix, iy,n_loop, fichier,gif
    ...
    tpi_s = tpi = round( clip.end*1000/ nb_frame  )
    clip.close()
    s_tpi.set(tpi)
...
def f_sauvegarde_webp():
    global fichier
    if (not nb_frame): return
    fich = asksaveasfilename(title = "Choisier un fichier",initialdir=fichier[:-4-len(nom)], initialfile=Nom(fichier) ,defaultextension=".webp") # ,defaultextension=".gif" ,filetypes = (("fichier gif","*.gif"))
    if (fich == ""): return
    fichier=fich
    gif = []
    x = int(ix*resolution)
...
img_charge = ImageTk.PhotoImage( Image.open(patch + 'charge.jpg') )
img_vide = ImageTk.PhotoImage( Image.open(patch + 'vide.jpg') )
